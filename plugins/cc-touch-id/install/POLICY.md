# cc-touch-id-gate policy reference

`/opt/cc-touch-id-gate/policy.json` is the **best-effort hook** policy: it decides which agent tool
calls require a Touch ID presence check. It is root-owned and the agent cannot edit it. (The *hard*
guarantee — `_cctouchid`-owned + `uchg` custody — is separate and stronger; this policy is the broad,
honestly-defeatable tier.)

## How to configure

Edit `plugins/cc-touch-id/install/policy.json` before install, **or** point the installer at your own
file: `POLICY=/path/to/policy.json sudo cc-touch-id install --policy /path/to/policy.json`. Install
substitutes `__HOME__` with your home, validates, and installs atomically — a broken or blanket policy
aborts the install and leaves the previous one intact. Check a file yourself any time with
`/opt/cc-touch-id-gate/cc-touch-id _validate-policy <file>`.
